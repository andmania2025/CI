...
const NAVIGATION_STYLES = {
  trigger: "text-black focus:text-black data-[state=open]:text-black text-base font-semibold h-8",
  ...
  dropdown: "grid w-[300px] gap-0 p-2 bg-white border border-gray-200 rounded-md shadow-lg",
  listItem:
    ...
    return (
      <li>
        <NavigationMenuLink asChild>
          <Link ref={ref} className={cn(NAVIGATION_STYLES.listItem, className)} {...props}>
...
const DropdownMenu: React.FC<DropdownMenuProps> = ({ title, items }) => (
  <NavigationMenuItem>
    <NavigationMenuTrigger className={NAVIGATION_STYLES.trigger}>{title}</NavigationMenuTrigger>
    <NavigationMenuContent>
...
export function MainNavigation({ className }: MainNavigationProps) {
  return (
    <NavigationMenu className={cn("h-8", className)} viewport={false} delayDuration={0}>
