...
import { SiteInquiryDetailModal } from "./SiteInquiryDetailModal";
...
interface SiteInquiryTableProps {
...
export const SiteInquiryTable: React.FC<SiteInquiryTableProps> = ({
  inquiries,
  selectedInquiries,
  selectAll,
  onSelectAll,
  ...
  const handleDetailClick = (inquiry: SiteInquiry) => {
  ...
  // セル内容のレンダリング
  const renderCellContent = (column: ColumnDef, inquiry: SiteInquiry) => {
    switch (column.key) {
      case "date":
      ...
      case "inquiryType":
        return <span className="truncate">{inquiry.inquiryType}</span>;
      case "status":
        return (
